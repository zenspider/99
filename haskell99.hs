...
tests =
  test
    ...
    , [1..5]                            ~=? myFlatten (List [Elem 1, List [Elem 2, List [Elem 3, Elem 4], Elem 5]])
    , ([]::[Int])                       ~=? myFlatten (List []) -- TODO: why the type hint?
    , "abcade"                          ~=? compress testUnencoded
    , ["aaaa","b","cc","aa","d","eeee"] ~=? pack testUnencoded
    , testPacked                        ~=? encode testUnencoded
    , testEncoded                       ~=? encodeModified testUnencoded
    , testUnencoded                     ~=? decodeModified testEncoded
    , testEncoded                       ~=? encodeDirect testUnencoded
    , [1,1,2,2,3,3]                     ~=? dupli [1, 2, 3]
    , "aaabbbccc"                       ~=? repli 3 "abc"
    ...
    , "fdacebgh"                        ~=? rndSelect "abcdefgh" 8 (mkStdGen 42)
    , [43,41,12,46,11,19]               ~=? randomList 6 49 (mkStdGen 42)
    , "fdbeca"                          ~=? randomPerm "abcdef" (mkStdGen 42)
    , ["abc","abd","acd","bcd"]         ~=? combinations 3 "abcd"
    ...
    , ["o","de","de","mn","abc","fgh","ijkl"] ~=? lsort ["abc","de","fgh","de","ijkl","mn","o"]

    , ["ijkl","o","abc","fgh","de","de","mn"] ~=? lfsort ["abc", "de", "fgh", "de", "ijkl", "mn", "o"]
