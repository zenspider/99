...
#     Given a run-length code list generated as specified in problem P11. Construct its
...
#
# P26 (**) Generate the combinations of K distinct objects chosen from the N elements of
#     a list
...
#     and 1, both lengths appear just once. The third and forth list have length 3 which
#     appears twice (there are two list of this length). And finally, the last three
...
#     phi(m) can be efficiently calculated as follows: Let ((p1 m1) (p2 m2) (p3 m3) ...)
#     be the list of prime factors (and their multiplicities) of a given number m. Then
#     phi(m) can be calculated with the following formula:
#
#     phi(m) = (p1 - 1) * p1 ** (m1 - 1) + (p2 - 1) * p2 ** (m2 - 1) + (p3 - 1) * p3 **
#     (m3 - 1) + ...
#
...
#     number of logical inferences as a measure for efficiency. Try to calculate phi
#     (10090) as an example.
#
...
#     has been numerically confirmed up to very large numbers (much larger than we can
...
#     of them is very small. Very rarely, the primes are both bigger than say 50. Try to
#     find out how many such cases there are in the range 2..3000.
#
...
#     operations; e.g. and(A,B) will succeed, if and only if both A and B succeed. Note
#     that A and B can be Prolog goals (not only the constants true and fail).
#
#     A logical expression in two variables can then be written in prefix notation, as
#     in the following example: and(or(A,B),nand(A,B)).
#
...
#     Continue problem P46 by defining and/2, or/2, etc as being operators. This allows
#     to write the logical expression in the more natural way, as in the example: A and
...
#     the truth table for the expression Expr, which contains the logical variables
#     enumerated in List.
#
#     Example:
...
#
# P49 (**) Gray code.
#     An n-bit Gray code is a sequence of n-bit strings constructed according to certain
#     rules. For example,
#     n = 1: C(1) = ['0','1'].
#     n = 2: C(2) = ['00','01','11','10'].
...
#     for the symbol S. In our example, the result could be Hs = [hc(a,'0'), hc
#     (b,'101'), hc(c,'100'), hc(d,'111'), hc(e,'1101'), hc(f,'1100')] [hc
#     (a,'01'),...etc.]. The task shall be performed by the predicate huffman/2 defined
#     as follows:
#
...
# which are binary trees themselves. [p67]
# In Lisp we represent the empty tree by 'nil' and the non-empty tree by the list (X L
# R), where X denotes the root node and L and R denote the left and right subtree,
# respectively. The example tree depicted opposite is therefore represented by the
# following list:
#
...
#
# You can check your predicates using these example trees. They are given as test cases
# in p54.lisp.
#
# P54A (*) Check whether a given term represents a binary tree
#     Write a predicate istree which returns true if and only if its argument is a list
...
#     The number of nodes in its left subtree and the number of nodes in its right
...
#     image of another. We are only interested in the structure, not in the contents of
#     the nodes.
#
...
#     * sym-cbal-trees(5,Ts).
#     Ts = [t(x, t(x, nil, t(x, nil, nil)), t(x, t(x, nil, nil), nil)), t(x, t(x, t(x,
#     nil, nil), nil), t(x, nil, t(x, nil, nil)))]
#
...
#     Consider a height-balanced binary tree of height H. What is the maximum number of
#     nodes it can contain?
...
#     A leaf is a node with no successors. Write a predicate count-leaves/2 to count
#     them.
#
#     % count-leaves(T,N) :- the binary tree T has N leaves
#
# P61A (*) Collect the leaves of a binary tree in a list
#     A leaf is a node with no successors. Write a predicate leaves/2 to collect them in
#     a list.
#
...
#
#     Using atlevel/3 it is easy to construct a predicate levelorder/2 which creates the
...
#     note that we start counting the levels from 1 at the root). In level H, which may
#     contain less than the maximum possible number of nodes, all the nodes are
#     "left-adjusted". This means that in a levelorder tree traversal all internal nodes
#     come first, the leaves come second, and empty successors (the nil's which are not
#     really nodes!) come last.
#
...
#     enumerating the nodes in levelorder, starting at the root with number 1. In doing
#     so, we realize that for every node X with address A the following property holds:
#     The address of X's left and right successors are 2*A and 2*A+1, respectively,
#     supposed the successors do exist. This fact can be used to elegantly construct a
#     complete binary tree structure. Write a predicate complete-binary-tree/2 with the
...
#
#     [p64] In this layout strategy, the position of a node v is obtained by the
#     following two rules:
#
...
#     % t(W,X,Y,L,R) represents a (non-empty) binary tree with root W "positioned" at
#     (X,Y), and subtrees L and R
#
#     Write a predicate layout-binary-tree/2 with the following specification:
#
#     % layout-binary-tree(T,PT) :- PT is the "positioned" binary tree obtained from the
#     binary tree T. (+,?)
#
#     Test your predicate in an appropriate way.
#
# P65 (**) Layout a binary tree (2)
#     [p65] An alternative layout method is depicted in the illustration opposite. Find
...
#     [p66] Yet another layout strategy is shown in the illustration opposite. The
#     method yields a very compact layout while maintaining a certain symmetry in every
...
#     does this inverse; i.e. given the string representation, construct the tree in the
#     usual form. Finally, combine the two predicates in a single predicate tree-string/
#     2 which can be used in both directions.
#
#     b) Write the same predicate tree-string/2 using difference lists and a single
#     predicate tree-dlist/2 which does the conversion between a tree and a difference
#     list in both directions.
#
#     For simplicity, suppose the information in the nodes is a single letter and there
...
#     pre-in-tree/3 that does the job.
#
...
#     subtree (nil) is encountered during the tree traversal. For example, the tree
#     shown in problem P67 is represented as 'abd..e..c.fg...'. First, try to establish
#     a syntax (BNF or syntax diagrams) and then write a predicate tree-dotstring/2
...
# which are multiway trees themselves. A multiway tree is never empty. The set of
# successor trees is sometimes called a forest.
#
# [p70]
# In Prolog we represent a multiway tree by a term t(X,F), where X denotes the root node
# and F denotes the forest of successor trees (a Prolog list). The example tree depicted
...
#     term representing a multiway tree.
#     Example:
...
#
# P70 (**) Tree construction from a node string [p70]
...
#     lengths from the root to all nodes of the tree. By this definition, the tree in
#     the figure of problem P70 has an internal path length of 9. Write a predicate ipl
#     (Tree,IPL) for the flow pattern (+,-).
#
# P72 (*) Construct the bottom-up order sequence of the tree nodes
#     Write a predicate bottom-up(Tree,Seq) which constructs the bottom-up sequence of
#     the nodes of the multiway tree Tree. Seq should be a Prolog list. What happens if
#     you run your predicate backwords?
#
...
#     ')', which we shall collectively call "tokens". We can represent this sequence of
#     tokens as a Prolog list; e.g. the lispy expression (a (b c)) could be represented
#     as the Prolog list ['(', a, '(', b, c, ')', ')']. Write a predicate tree-ltl
#     (T,LTL) which constructs the "lispy token list" LTL if the tree is given as term T
...
# nodes.
#
# There are several ways to represent graphs in Prolog. One method is to represent each
# edge separately as one clause (fact). In this form, the graph depicted below is
# represented as the following predicate: [graph1]
#
# edge(h,g).
# edge(k,f).
# edge(f,b).
...
# Another method is to represent the whole graph as one data object. According to the
# definition of the graph as a pair of two sets (nodes and edges), we may use the
# following Prolog term to represent the example graph:
#
# graph([b,c,d,f,g,h,k],[e(b,c),e(b,f),e(c,f),e(f,k),e(g,h)])
#
# We call this graph-term form. Note, that the lists are kept sorted, they are really
# sets, without duplicated elements. Each edge appears only once in the edge list; i.e.
# an edge from a node x to another node y is represented as e(x,y), the term e(y,x) is
# not present. The graph-term form is our default representation. In SWI-Prolog there
# are predefined predicates to work with sets.
#
# A third representation method is to associate with each node the set of nodes that are
# adjacent to that node. We call this the adjacency-list form. In our example:
#
# [n(b,[c,f]), n(c,[b,f]), n(d,[]), n(f,[b,c,k]), ...]
#
# The representations we introduced so far are Prolog terms and therefore well suited
# for automated processing, but their syntax is not very user-friendly. Typing the terms
# by hand is cumbersome and error-prone. We can define a more compact and
# "human-friendly" notation as follows: A graph is represented by a list of atoms and
# terms of the type X-Y (i.e. functor '-' and arity 2). The atoms stand for isolated
# nodes, the X-Y terms describe edges. If an X appears as an endpoint of an edge, it is
# automatically defined as a node. Our example could be written as:
#
# [b-c, f-c, g-h, d, f-b, k-f, h-g]
#
# We call this the human-friendly form. As the example shows, the list does not have to
# be sorted and may even contain the same edge multiple times. Notice the isolated node
# d. (Actually, isolated nodes do not even have to be atoms in the Prolog sense, they
# can be compound terms, as in d(3.75,blue) instead of d in the example).
#
# [graph2] When the edges are directed we call them arcs. These are represented by
# ordered pairs. Such a graph is called directed graph. To represent a directed graph,
...
#
# Finally, graphs and digraphs may have additional information attached to nodes and
# edges (arcs). For the nodes, this is no problem, as we can easily replace the single
# character identifiers with arbitrary compound terms, such as city('London',4711). On
# the other hand, for edges we have to extend our notation. Graphs with additional
...
#     [p>q/9, m>q/7, k, p>m/5]
#
# The notation for labelled graphs can also be used for so-called multi-graphs, where
# more than one edge (or arc) are allowed between two given nodes.
#
# P80 (***) Conversions
...
#     Write a predicate path(G,A,B,P) to find an acyclic path P from node A to node b in
#     the graph G. The predicate should return all paths via backtracking.
#
# P82 (*) Cycle from a given node
#     Write a predicate cycle(G,A,P) to find a closed path (cycle) P starting at a given
#     node A in the graph G. The predicate should return all cycles via backtracking.
#
# [p83]
#
# P83 (**) Construct all spanning trees
#     Write a predicate s-tree(Graph,Tree) to construct (by backtracking) all spanning
#     trees of a given graph. With this predicate, find out how many spanning trees
#     there are for the graph depicted to the left. The data of this example graph can
#     be found in the file p83.dat. When you have a correct solution for the s-tree/2
#     predicate, use it to define two other useful predicates: is-tree(Graph) and
#     is-connected(Graph). Both are five-minutes tasks!
#
# [p84]
#
# P84 (**) Construct the minimal spanning tree
#     Write a predicate ms-tree(Graph,Tree,Sum) to construct the minimal spanning tree
#     of a given labelled graph. Hint: Use the algorithm of Prim. A small modification
#     of the solution of P83 does the trick. The data of the example graph to the right
#     can be found in the file p84.dat.
#
# P85 (**) Graph isomorphism
#     Two graphs G1(N1,E1) and G2(N2,E2) are isomorphic if there is a bijection f: N1 ->
#     N2 such that for any nodes X,Y of N1, X and Y are adjacent if and only if f(X) and
#     f(Y) are adjacent.
#
#     Write a predicate that determines whether two graphs are isomorphic. Hint: Use an
#     open-ended list to represent the function f.
#
...
#
#     c) Use Welch-Powell's algorithm to paint the nodes of a graph in such a way that
#     adjacent nodes have different colors.
#
# P87 (**) Depth-first order graph traversal (alternative solution)
#     Write a predicate that generates a depth-first order graph traversal sequence. The
#     starting point should be specified, and the output should be a list of nodes that
#     are reachable from this starting point (in depth-first order).
#
...
#     two queens are in the same row, the same column, or on the same diagonal.
#
#     Hint: Represent the positions of the queens as a list of numbers 1..N. Example:
#     [4,2,7,3,6,8,5,1] means that the queen in the first column is in row 4, the queen
#     in the second column is in row 2, etc. Use the generate-and-test paradigm.
#
# P91 (**) Knight's tour
#     Another famous problem is this one: How can a knight jump on an NxN chessboard in
#     such a way that it visits every square exactly once?
#
...
#     functor, not division!) Define the relation jump(N,X/Y,U/V) to express the fact
#     that a knight can jump from X/Y to U/V on a NxN chessboard. And finally, represent
#     the solution of our problem as a list of N*N knight positions (the knight's tour).
#
# P92 (***) Von Koch's conjecture
#     Several years ago I met a mathematician who was intrigued by a problem for which
#     he didn't know a solution. His name was Von Koch, and I don't know whether the
#     problem has been solved since.
#
#     [p92a] [p92b]
#
#     Anyway the puzzle goes like this: Given a tree with N nodes (and hence N-1 edges).
#     Find a way to enumerate the nodes from 1 to N and, accordingly, the edges from 1
#     to N-1 in such a way, that for each edge K the difference of its node numbers
#     equals to K. The conjecture is that this is always possible.
#
#     For small trees the problem is easy to solve by hand. However, for larger trees,
#     and 14 is already very large, it is extremely difficult to find a solution. And
#     remember, we don't know for sure whether there is always a solution!
#
#     Write a predicate that calculates a numbering scheme for a given tree. What is the
#     solution for the larger tree pictured above?
#
...
#     numbers [2,3,5,7,11] we can form the equations 2-3+5+7 = 11 or 2 = (3*5+7)/11 (and
#     ten others!).
#
...
#     nodes are there? See also a table of results and a Java applet that can represent
...
#     [p96] In a certain programming language (Ada) identifiers are defined by the
#     syntax diagram (railroad chart) opposite. Transform the syntax diagram into a
#     system of syntax diagrams which do not contain loops; i.e. which are purely
#     recursive. Using these modified diagrams, write a predicate identifier/1 that can
...
#
#     Every spot in the puzzle belongs to a (horizontal) row and a (vertical) column, as
#     well as to one single 3x3 square (which we call "square" for short). At the
#     beginning, some of the spots carry a single-digit number between 1 and 9. The
#     problem is to fill the missing spots with digits in such a way that every number
#     between 1 and 9 appears exactly once in each row, in each column, and in each
#     square.
# P98 (***) Nonograms
#     Around 1994, a certain kind of puzzles was very popular in England. The "Sunday
#     Telegraph" newspaper wrote: "Nonograms are puzzles from Japan and are currently
#     published each week only in The Sunday Telegraph. Simply use your logic and skill
#     to complete the grid and reveal a picture or diagram." As a Prolog programmer, you
#     are in a better situation: you can have your computer do the work! Just write a
#     little program ;-).
#
...
#                2 1 5 1                     2 1 5 1
#
#     For the example above, the problem can be stated as the two lists [[3],[2,1],
#     [3,2],[2,2],[6],[1,5],[6],[1],[2]] and [[1,2],[3,1],[1,5],[7,1],[5],[3],[4],[3]]
#     which give the "solid" lengths of the rows and columns, top-to-bottom and
#     left-to-right, respectively. Published puzzles are larger than this example, e.g.
#     25 x 20, and apparently always have unique solutions.
#
...
#
#     [p99] The particular crossword puzzle is specified in a text file which first
#     lists the words (one word per line) in an arbitrary order. Then, after an empty
#     line, the crossword framework is defined. In this framework specification, an
#     empty character location is represented by a dot (.). In order to make the
#     solution easier, character locations can also contain predefined character values.
#     The puzzle opposite is defined in the file p99a.dat, other examples are p99b.dat
#     and p99d.dat. There is also an example of a puzzle (p99c.dat) which does not have
#     a solution.
#
#     Words are strings (character lists) of at least two characters. A horizontal or
#     vertical sequence of character places in the crossword puzzle framework is called
...
#     understand it. So, don't give up too early! And remember that the objective is a
#     clean solution, not just a quick-and-dirty hack!
#     (2) Reading the data file is a tricky problem for which a solution is provided in
#     the file p99-readfile.lisp. Use the predicate read_lines/2.
#     (3) For efficiency reasons it is important, at least for larger puzzles, to sort
#     the words and the sites in a particular order. For this part of the problem, the
#     solution of P28 may be very helpful.
